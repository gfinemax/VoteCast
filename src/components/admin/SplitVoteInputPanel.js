...
const SPLIT_VOTE_GRID_CLASS = 'md:grid-cols-[72px_minmax(40px,0.45fr)_16px_minmax(96px,1.15fr)_16px_minmax(84px,0.75fr)]';
const ONSITE_INPUT_HEADER_WIDTH_CLASS = 'w-[82%] mx-auto';
const ONSITE_INPUT_FIELD_WIDTH_CLASS = 'w-full md:w-[82%] md:mx-auto';
    ...
    return (
        <div className="flex flex-col flex-1 mt-0">
            <div className="rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2 shadow-[0_12px_40px_-15px_rgba(0,0,0,0.5)] flex-1">
                <div className="mb-2 flex flex-col gap-2 md:flex-row md:items-center md:justify-between border-b border-slate-800 pb-2">
                ...
                <div className={`hidden ${SPLIT_VOTE_GRID_CLASS} gap-1 px-2 pb-3 text-[11px] font-bold uppercase tracking-wide text-slate-500 md:grid items-center`}>
                ...
                <div className="space-y-2">
                    {splitVoteDisplayCards.map((card) => (
                        <div
                            key={`${card.key}-row`}
                            className={`grid grid-cols-1 ${SPLIT_VOTE_GRID_CLASS} gap-2 md:gap-1 items-center rounded-2xl border px-3 py-1.5 md:px-2 transition-colors overflow-hidden ${card.tone.rowTint} hover:shadow-md`}
                        >
                            <div className="flex items-center justify-between md:flex md:justify-center">
                                <span className={`inline-flex h-12 w-[80px] items-center justify-center rounded-lg border px-2 text-sm font-bold shadow-sm md:h-10 md:w-[64px] md:px-1 md:text-xs xl:h-12 xl:w-[72px] xl:text-sm ${card.tone.labelBadge}`}>
                                    {card.summaryLabel.replace('전체 ', '')}
                                </span>
                            </div>

                            <div className="flex flex-col items-center justify-between rounded-xl bg-slate-800/30 px-4 py-3 md:bg-transparent md:px-0 md:py-0 md:justify-center">
                                <span className="text-xs font-bold text-slate-500 md:hidden mb-1">{fixedVoteLabel}</span>
                                <span className={`font-mono text-xl md:text-2xl font-black tabular-nums ${card.tone.writtenText}`}>{card.writtenValue}</span>
                            </div>

                            <div className="hidden md:flex justify-center text-slate-700 text-xl font-black leading-none xl:text-2xl">+</div>

                            <div className={`relative md:flex md:justify-center ${isQuorumLocked ? 'opacity-35' : ''}`}>
                                <div className={`mb-1 text-xs font-bold md:hidden ${card.tone.inputLabel}`}>현장 참석입력</div>
                                <VoteNumericInput
                            ...
                            <div className="min-w-0 flex flex-col items-center justify-between rounded-xl bg-slate-800/30 px-4 py-3 md:bg-transparent md:px-0 md:py-0 md:justify-center">
                                <span className="text-xs font-bold text-slate-500 md:hidden mb-1">최종 득표</span>
                                <div className="flex min-w-0 w-full items-end justify-center gap-1 whitespace-nowrap">
                                    <span className={`min-w-0 font-mono text-xl md:text-[22px] xl:text-2xl font-black tabular-nums leading-none ${card.tone.totalText}`}>{card.totalValue}</span>
