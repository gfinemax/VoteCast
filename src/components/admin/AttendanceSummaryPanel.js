        ...
        <section className={`flex flex-col flex-1 ${isConfirmed ? "opacity-90 grayscale-[0.3]" : ""}`}>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3 shadow-xl relative overflow-hidden ring-1 ring-white/5 group flex flex-col gap-3 flex-1">
                <div className="flex items-center justify-between bg-emerald-950/40 border border-emerald-900/50 rounded-xl p-2.5 px-3 mb-2 transition-colors group-hover:bg-emerald-950/60 group-hover:border-emerald-800/60">
                    <div className="text-emerald-400 font-bold text-[11px] tracking-widest uppercase flex flex-col leading-tight">
                        <span>성원합계</span>
                        <span className="text-[10px] text-emerald-500/90 font-semibold">(개최기준{quorumTarget}명)</span>
                    </div>
                ...
                <div className="grid grid-cols-[1.1fr_auto_1.6fr] gap-1 mb-1.5 px-0.5 text-[10px] font-bold text-slate-400 text-center tracking-wide items-end">
                    <div className="text-slate-300">{fixedAttendanceLabel}</div>
                    <div className="w-3"></div>
                    <div className="flex flex-col items-center leading-tight">
                        <span className="text-slate-300">{isElection ? '현장 선거 가능' : '현장 참석'}</span>
                        <span className="text-[9px] font-semibold text-slate-400">{isElection ? `(총 현장 ${totalOnsiteAttendance}명 중 대리 ${proxyCount}명 제외)` : '(조합원+대리인)'}</span>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between mb-3 px-1">
                    ...
                    <div className="flex-[1.6] flex items-stretch border border-blue-900/50 bg-blue-900/20 rounded-xl overflow-hidden shadow-inner relative transition-colors group-hover:border-blue-800/60">
                        <div className="flex-1 flex flex-col items-center justify-center py-2 relative">
                            <span className="text-[10px] font-bold text-blue-400 absolute top-0.5">조합원</span>
                        ...
                        <div className={`flex-1 flex flex-col items-center justify-center py-2 relative ${isElection ? 'bg-amber-950/30' : ''}`}>
                            <span className={`text-[10px] font-bold absolute top-0.5 ${isElection ? 'text-amber-400' : 'text-blue-400'}`}>{isElection ? '대리인 제외' : '대리인'}</span>
                ...
                <div className="bg-slate-950/50 p-2.5 rounded-xl border border-slate-800 transition-colors group-hover:bg-slate-950/70 mt-1">
                    ...
                    <div className="flex justify-between items-center text-[10px]">
                        <span className="text-slate-400">
                            개회 기준: <span className="text-slate-200 font-bold">{quorumTarget}명</span>
                            {isElection && <span className="text-emerald-400/90 ml-1">중에 현장참석 <span className="font-bold underline underline-offset-2">{directTarget}명</span>(20%)</span>}
                        </span>
