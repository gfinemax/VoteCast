...
const buildInitialElectionVotes = (agendas = []) => {
    const initialVotes = {};
    agendas.forEach((agenda) => {
        initialVotes[agenda.id] = getDefaultElectionChoice(agenda, agendas);
    });
    return initialVotes;
};
...
const formatCheckInSummary = (state = {}, options = {}) => {
    const lines = [`총회 상태: ${MEETING_TYPE_LABELS[state.meetingType] || '미등록'}`];

    if (options.hasElectionAgenda) {
        lines.push(`선거 상태: ${ELECTION_MODE_LABELS[state.electionMode] || '미등록'}`);
    }
    ...
    if (options.hasElectionAgenda && (state.electionVoteCount || 0) > 0) {
        lines.push(`우편투표 응답: ${state.electionVoteCount}건`);
    }

    return lines.join('\n');
};
const buildTransitionPrompt = ({ member, currentState, nextState, hasElectionAgenda = true }) => {
    ...
    const remoteSubmissionLabel = hasElectionAgenda ? '서면결의 또는 우편투표' : '서면결의';

    if (existingHasRemote && nextHasOnsite) {
        return {
            title: '기존 제출 내역이 있습니다',
            message: `${memberLabel} 조합원은 이미 ${remoteSubmissionLabel}가 접수되어 있습니다.\n현재 입력값을 적용하면 기존 제출 내역과 현장 상태가 함께 정리되어 최종값으로 다시 반영됩니다.\n\n현재 저장 상태\n${formatCheckInSummary(currentState, { hasElectionAgenda })}\n\n변경 예정 상태\n${formatCheckInSummary(nextState, { hasElectionAgenda })}`,
...
const getAttendanceBadges = (record, options = {}) => {
    ...
    if ((options.showElectionBadge ?? true) && (record.has_election || electionMode !== 'none')) {
...
export default function CheckInPage() {
    ...
    const chairContestAgendas = useMemo(
        () => electionAgendas.filter((agenda) => isChairContestAgenda(agenda, electionAgendas)),
        [electionAgendas]
    );
    const isChairContestSelectionValid = (
        checkInForm.electionMode !== 'mail'
        || chairContestAgendas.length < 2
        || chairContestAgendas.filter((agenda) => checkInForm.electionVotes?.[agenda.id] === 'yes').length === 1
    );
    ...
    const handleElectionVoteChoice = (agenda, choice) => {
        setCheckInForm((prev) => {
            const nextVotes = { ...prev.electionVotes, [agenda.id]: choice };

            if (choice === 'yes' && isChairContestAgenda(agenda, electionAgendas)) {
                chairContestAgendas.forEach((chairAgenda) => {
                    if (chairAgenda.id !== agenda.id) {
                        nextVotes[chairAgenda.id] = 'no';
                    }
                });
            }
                        ...
                        <div className="px-4 py-2 bg-white">
                            ...
                            {isStatsOpen && (
                                <div className="mt-1 space-y-3 border-t border-slate-100 pt-2 animate-in slide-in-from-top-2 duration-200">
                                    <div className={`grid grid-cols-2 ${hasElectionAgenda ? 'md:grid-cols-4' : 'md:grid-cols-3'} gap-2 text-center`}>
                                    ...
                                    <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-600 flex items-center justify-between">
                                        <span>{hasElectionAgenda ? '총회/선거 전체 처리' : '총회 전체 처리'}</span>
                                        <span className="text-slate-900">{hasElectionAgenda ? stats.participantCount : stats.checkedIn}명</span>
                                    </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-2 pb-40">
                    {filteredMembers.map(member => {
                        ...
                        const badges = getAttendanceBadges(record, {
                            ...
                            showElectionBadge: hasElectionAgenda
                        });
                    ...
                    <div className="bg-white rounded-t-3xl md:rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in slide-in-from-bottom-8 md:zoom-in-95 duration-200 flex flex-col max-h-[92vh]">
                        <div className="p-5 border-b border-slate-100 bg-slate-50">
                            <div className="flex items-start justify-between gap-3">
                                <div>
                                    <h3 className="text-lg font-bold text-slate-800">{checkInModalMode === 'edit' ? (hasElectionAgenda ? '입장 및 선거 수정' : '입장 수정') : (hasElectionAgenda ? '입장 및 선거 접수' : '입장 접수')}</h3>
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-5">
                            ...
                            {checkInForm.meetingType === 'proxy' && hasElectionAgenda && (
                            ...
                            {hasElectionAgenda && checkInForm.electionMode === 'mail' && (
                                <section className="space-y-3">
                                    <div className="flex items-center justify-between gap-3">
                                        <div>
                                            <div className="text-sm font-bold text-slate-800">우편투표 입력</div>
                                            <p className="text-xs text-slate-500">경선은 후보 선택, 찬반 선거는 찬성·반대 기준으로 입력합니다.</p>
                                        </div>
                                        <button
                                            onClick={() => setCheckInForm((prev) => {
                                                const nextVotes = buildInitialElectionVotes(electionAgendas);
                                                return { ...prev, electionVotes: nextVotes };
                                            })}
                                                ...
                                                return (
                                                    <div key={agenda.id} className="flex flex-col gap-2">
                                                        <div>
                                                            ...
                                                            <span className="ml-2 align-middle text-[10px] font-black text-amber-700">
                                                                {rule.label}
                                                            </span>
                                                        </div>
                                                        <div className="flex bg-slate-100 rounded-lg p-1">
                                                            <button
                                                                onClick={() => handleElectionVoteChoice(agenda, 'yes')}
                                                            ...
                                                            >
                                                                {getElectionChoiceLabel(agenda, 'yes', electionAgendas)}
                                                            </button>
                                                            <button
                                                                onClick={() => handleElectionVoteChoice(agenda, 'no')}
                                                                className={`flex-1 py-2 rounded-md text-sm font-bold transition-all ${checkInForm.electionVotes[agenda.id] === 'no' ? `bg-white ${rule.isContest ? 'text-slate-600' : 'text-red-500'} shadow-sm ring-1 ring-slate-200` : 'text-slate-400 hover:text-slate-600'}`}
                                                            >
                                                                {getElectionChoiceLabel(agenda, 'no', electionAgendas)}
                                                            </button>
                                                            <button
                                                                onClick={() => handleElectionVoteChoice(agenda, 'abstain')}
                                                            ...
                                                            >
                                                                {getElectionChoiceLabel(agenda, 'abstain', electionAgendas)}
                                                            </button>
                                                        </div>
                                                    </div>
                                                );
                            ...
                            <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    {hasElectionAgenda && !isMailElectionComplete && (
                                        <span className="text-xs font-semibold text-red-500">우편투표를 선택한 경우 모든 선거 안건에 응답해야 합니다.</span>
                                    )}
                                    {hasElectionAgenda && checkInForm.electionMode === 'mail' && !isChairContestSelectionValid && (
                                        <span className="text-xs font-semibold text-red-500">조합장 경선 우편투표는 후보 1명만 선택해야 합니다.</span>
                                    )}
                                    {!isWrittenComplete && (
                                        <span className="text-xs font-semibold text-red-500">서면결의를 선택한 경우 모든 총회 안건에 응답해야 합니다.</span>
                                    )}
                                    {checkInForm.electionMode === 'none' && checkInForm.meetingType === 'none' && (
                                        <span className="text-xs font-semibold text-red-500">{hasElectionAgenda ? '총회 상태 또는 선거 참여를 하나 이상 선택해야 합니다.' : '총회 상태를 선택해야 합니다.'}</span>
                                    )}
                                </div>
                            </section>
                        </div>
